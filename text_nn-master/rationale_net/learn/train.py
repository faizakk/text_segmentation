...
def test_model(test_data, model, gen, args):
    ...
    test_stats['rationales'] = rationales;# print(rationales)
...
def run_epoch(data_loader, train_model, model, gen, optimizer, step, args):
    ...
    for _ in tqdm.tqdm(range(num_batches_per_epoch)):
        ...
        print(type(loss),loss,loss.shape); obj_loss = loss
	#print('debugging y, args and loss',y,args,loss)
        if args.get_rationales:
    ...
    epoch_metrics = metrics.get_metrics(preds, golds, args);print('This is it', rationales[:100])
...
def get_loss(logit,y, args):
    ...
    loss= loss.reshape([1]);return loss
